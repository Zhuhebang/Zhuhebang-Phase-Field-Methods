...
Another dependency is the mpi version of fftw library, we need the \textbf{libfftw3\_mpi.so.3} and \textbf{libfftw3.so.3} shared library. Make sure the fftw library is compiled with intel compiler and intel mpi instead of gcc compiler.
...
To install and use the program is simple, all you need to do is copy the \textcolor{red}{Ferroelectric.exe} file to your server and change the permission of the file to be executable. Notice here the file extension exe does not mean it is a Window's program. The \textcolor{red}{Ferroelectric.exe} is a linux executable, the extension is to differentiate it from other text files.
