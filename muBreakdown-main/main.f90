...
!circle(3) = 8.0         !random Radius
 ...
 circles(ic,3) = R1 
 circles(ic,4) = R2
 CALL iniconf(INT(circle(1)),INT(circle(2)),nz,R1,phi1)  ！INT向下取整
...
!   Ebreak(ii,jj,kk) = Em_break
 ...
 DO kk=1,nz
  DO jj=1,ny
   DO ii=1,nx
    IF((ABS(ii - nx/2) <= 6).AND.(ABS(jj - ny/2) < 1)) eta(ii,jj,kk) = 1.0 ！绝对值
   END DO
  END DO
 END DO
